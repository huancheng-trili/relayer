...
async function forwardToTezlink(data) {
    ...
    console.log(`Operation injected at level ${level}: https://shadownet.tezlink.tzkt.io/${operation.hash}`);
}
...
if (process.env.ETHERLINK_PRIVATE_KEY !== undefined) {
    etherlinkWallet = new ethers.Wallet(process.env.ETHERLINK_PRIVATE_KEY, etherlinkProvider);
}
const etherlinkAbi = [
    "function newIntent(bytes calldata data) public",
];

const Tezos = new TezosToolkit('https://rpc.shadownet.tezlink.nomadic-labs.com');
Tezos.setProvider({
